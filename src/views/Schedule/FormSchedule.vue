    ...
    <ValidationObserver ref="observer" v-slot="{ handleSubmit }">
      <form @submit.prevent="handleSubmit(onSaveClicked)">
        <div class="columns is-multiline">
          ...
          <div class="column is-9">
            <ValidationProvider name="startTime" rules="required" v-slot="{ errors }">
              <b-timepicker
          ...
          <div class="column is-9">
            <ValidationProvider name="endTime" rules="required" v-slot="{ errors }">
              <b-timepicker
...
export default {
  ...
  async created() {
    if (this.$route.name == 'Schedule.Edit') {
      ...
      let startTime = new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate() + " " + response.data.startTime
      let endTime = new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate() + " " + response.data.endTime
  ...
  methods: {
    ...
    async onSaveClicked() {
      let data = {
        ...
        end_time: new Date(this.data.endTime).getHours() + ":" + new Date(this.data.endTime).getMinutes() + ":" + new Date(this.data.endTime).getSeconds(),
      }
